...
本 Demo 展示 UniRTOS 上 USB 网络共享（USBNET）功能，覆盖模式配置、网络附着、PDP 数据连接以及 USB 网络共享的启动/停止流程。

- 演示读取当前 USBNET 模式，在 RNDIS 与 ECM 间切换并触发设备重启
- 演示带超时的网络附着等待
- 演示 PDP 上下文配置：APN 与 IP 类型设置
- 演示在启用 USB 网络共享前先建立数据连接
- 演示通过异步回调方式启动和停止 USBNET
